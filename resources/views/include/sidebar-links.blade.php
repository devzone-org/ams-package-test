...
<nav class="flex-1 px-2 mt-5 space-y-1 bg-white">
    <a href="{{ url('accounts') }}"
       class="{{ Request::segment(1)=='accounts' && empty(Request::segment(2))? $a_current : '' }} group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:text-gray-900 hover:bg-gray-100">
    ...
    <div @if(Request::segment(1)=='accountant') x-data="{ isExpanded: true }" @else x-data="{ isExpanded: false }"
         @endif x-cloak
         class="space-y-1">
        <button
